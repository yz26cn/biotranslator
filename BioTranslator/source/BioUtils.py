...
def RandomWalkRestart(A, rst_prob, delta=1e-4, reset=None, max_iter=50, return_torch=False):
    ...
    for i in range(1, max_iter):
        ...
        if delta < 1e-4:
            break
    if device == 'cuda' and not return_torch:
        Q = Q.cpu().numpy()
    return Q
...
def get_clusDCA_emb(cfg):
    ...
    sp = RandomWalkRestart(adjacent_GO, 0.8, return_torch=True)
    print("finish rwr")
