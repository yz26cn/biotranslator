...
class BioTrainer:
    ...
    def setup_model(self, files: BioLoader, cfg: BioConfig):
        ...
        elif cfg.method in ['ProTranslator', 'TFIDF', 'clusDCA', 'Word2Vec', 'Doc2Vec']:
            self.model = BaseModel(cfg)
        elif cfg.method == 'DeepGoPlus':
            self.model = DeepGOPlus(cfg, files.n_classes)
        else:
            print("Warning: No such method to setup the model!")
    ...
    def setup_training(self, files: BioLoader, cfg: BioConfig):
        ...
        # Adam optimizer
        self.optimizer = torch.optim.Adam(self.model_parameters(), lr=cfg.lr)
