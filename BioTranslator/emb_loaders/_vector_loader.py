"""
gene expression vector encoder
"""
import os
import numpy as np
import pandas as pd
...
class VecLoader:
    ...
    def generate_data(self, iter=None, unseen_ratio=None):
        if self.cfg.task == 'same_dataset':
            ...
            test_feature, mapping = self.DataProcess_obj.ProcessTestFeature(test_feature, test_genes,
                                                                            log_transform=False)
            ...
            self.train_data = cellData(train_feature, train_label, nlabel=len(self.train2i.keys()),
                                       gpu_ids=self.cfg.gpu_ids)
            self.test_data = cellData(eval_feature, test_label, nlabel=len(self.test2i.keys()),
                                      gpu_ids=self.cfg.gpu_ids)
            self.train_emb = emb2tensor(self.text_embeddings, self.train2i)
            if len(self.cfg.gpu_ids) > 0:
                self.train_emb = self.train_emb.cuda()
